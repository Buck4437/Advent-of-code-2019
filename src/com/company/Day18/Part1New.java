...
public class Part1New {
    ...
    public static HashMap<String, Edge> constructMap(char[][] input) {
        ...
        for (int x = 0; x < input.length; x++) {
            ...
            for (int y = 0; y < row.length; y++) {
                ...
                switch (getType(c)) {
                    case KEYS -> keys.add(c);
                    case DOORS -> doors.add(c);
                }
                if (getType(c) != NONE && getType(c) != DOORS) {
        ...
        for (char start: checkpoints.keySet()) {
            for (char end: checkpoints.keySet()) {
                ...
                Edge edge = findPath(grid, checkpoints.get(start), checkpoints.get(end));
                map.put(key, edge);
                map.put(key2, edge);
            }
        }
    ...
    public static Edge findPath(HashMap<String, Character> grid, int[] start, int[] end) {
